...
func _process(_delta) -> void: #similiar to tick in UE4
	if not complete and call_started: #if complete false play level
		if main_scene.sockets[0].state == Socket.State.OFF: 
			if main_scene.sockets[1].state == Socket.State.OFF:
